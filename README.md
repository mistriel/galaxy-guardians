...
| Ground battles | קרבות קרקע drops into יער זוהר. Walk with arrows or WASD, shoot with Space. חייל, טנק, and המשמיד each send that unit into the fight. נסיגה pulls back. Every three wins, שומר החומה arrives; beating him flips a tarot card (חפיסת טאסו) with a gift for later battles |
| Pause | `Esc` or `P` |
